...
const TimelineView = memo(({ phases, currentPhaseIndex, completedPhases, onPhaseSelect }) => {
  return (
    <div className="mb-12 relative" role="navigation" aria-label="Election phases timeline">
          ...
          return (
            <div key={phase.id} className="flex flex-col items-center">
              <button
                ...
                disabled={isLocked}
                aria-label={`${phase.label}${isCompleted ? ' (completed)' : ''}${isActive ? ' (current)' : ''}${isLocked ? ' (locked)' : ''}`}
                ...
                `}
              >
                {isCompleted ? <Check className="w-6 h-6 stroke-[3]" aria-hidden="true" /> : phase.icon}
              </button>

              <div
                className={`mt-4 text-[10px] font-black uppercase tracking-[0.2em] transition-colors duration-500 ${isActive ? 'text-primary-600' : 'text-slate-400'}`}
                aria-hidden="true"
              >
                {phase.label}
              </div>
